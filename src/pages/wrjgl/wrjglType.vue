...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      loadData() {
        var self = this;
        var parentId = self.wrjgl.parentId;
        if(!parentId){
            Toast("请先选择无人机厂商！");
        }
        var xhr = api.getDronemList({
          "type":2,
          "droneM.parentId":parentId,
      ...
      getSelecItem(item){
        ...
        this.typeId = item.droneMId;
      },
      submit (){
        this.RECORD_WRJGLTYPE({"type":this.selectedItem,"typeId":this.typeId});
    ...
    created(){
      this._initPage();
      this.selectedItem = this.wrjgl.chooseType;
      this.typeId = this.wrjgl.typeId;
